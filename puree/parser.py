...
class UI:
        ...
        def load_container(container_data, parent_container):
            for attr_name, attr_value in container_data.items():
                if isinstance(attr_value, dict):
                    has_component_data = (
                        "data" in attr_value
                        and isinstance(attr_value["data"], str)
                        and attr_value["data"].startswith("[")
                        and attr_value["data"].endswith("]")
                    )
    ...
    def parse_css(self):
        ...
        _md_defaults_path = os.path.join(
            os.path.dirname(__file__),
            "components",
            "defaults",
            "markdown_defaults.scss",
        )
            ...
            for container_id, props in resolved.items():
                ...
                for prop, value in props.items():
                    ...
                    if (
                        isinstance(result, tuple)
                        and len(result) == 2
                        and result[0] in ("_transitions", "_scrollbar_color")
                    ):
                        continue
                    attr_name, attr_value = result
                    state_attr = (
    ...
    def create_node_tree(self, canvas_size):
        ...
        vw_unit = canvas_size[0] / 100.0
        vh_unit = canvas_size[1] / 100.0
        vmin_unit = min(canvas_size[0], canvas_size[1]) / 100.0
        vmax_unit = max(canvas_size[0], canvas_size[1]) / 100.0
        root_font_size = 16.0
    ...
    def _container_to_dict(self, container):
        ...
        container_dict = {
            ...
            "font": str(container.style.font_family).strip().strip("'\"")
