...
class KeyboardHandler(bpy.types.Operator):
    ...
    def modal(self, context, event):
            ...
            if event.value == "PRESS":
                try:
                    ...
                    consumed = key_manager.dispatch(
                        event, focused_container_id=focus_manager.get_focused_id(), any_input_focused=False
                    )
                    if consumed:
                        return {"RUNNING_MODAL"}
                except Exception:
        ...
        try:
            ...
            consumed = key_manager.dispatch(
                event, focused_container_id=focus_manager.get_focused_id(), any_input_focused=True
            )
            if consumed:
                return {"RUNNING_MODAL"}
        except Exception:
...
class CreateTextInputOP(bpy.types.Operator):
    ...
    selection_color: bpy.props.FloatVectorProperty(
        name="Selection Color", subtype="COLOR", size=4, default=(0.3, 0.5, 0.8, 0.3)
    )
