...
class Container:
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            for child in children:
                result = self._find_descendant(child, name)
    ...
    @staticmethod
    def _find_descendant(container, name):
        for child in container.children:
            if child.id == name or child.id.endswith(f"_{name}"):
                return child
        for child in container.children:
            result = Container._find_descendant(child, name)
            if result is not None:
                return result
        return None
            ...
            if self._layout_node is not None:
                ...
                if name == "width" or name == "height":
                    ...
                    if current_margin:
                        top = (
                            current_margin.top
                            if hasattr(current_margin, "top")
                            else LengthPointsPercentAuto.from_any(0 * PT)
                        )
                        right = (
                            current_margin.right
                            if hasattr(current_margin, "right")
                            else LengthPointsPercentAuto.from_any(0 * PT)
                        )
                        bottom = (
                            current_margin.bottom
                            if hasattr(current_margin, "bottom")
                            else LengthPointsPercentAuto.from_any(0 * PT)
                        )
                        left = (
                            current_margin.left
                            if hasattr(current_margin, "left")
                            else LengthPointsPercentAuto.from_any(0 * PT)
                        )
                    else:
