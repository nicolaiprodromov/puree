...
class RenderPipeline:
    ...
    def _draw_scrollbars(self):
        ...
        scroll_indices = set()
        for i, c in enumerate(containers):
            if c.get("display", False) and c.get("overflow_type", "VISIBLE") in (
                "SCROLL",
                "AUTO",
            ):
        ...
        if self.mgl_context:
            try:
                ...
                if "'InvalidObject' object has no attribute 'release'" not in str(e):
                    logger.warning(
                        "Unexpected AttributeError during ModernGL context GC",
                        exc_info=True,
                    )
            except Exception:
    ...
    def _build_container_struct(self, container):
        ...
        return [
            ...
            container.get("border_radius_tl", br),
            container.get("border_width", 0.0),
            container.get("border_radius_tr", br),
            container.get("border_radius_br", br),
            shadow_offset[0],
            shadow_offset[1],
            shadow_offset[2],
    ...
    def _get_scroll_clip_for_container(self, idx, containers):
        ...
        if has_clip:
            return (
                int(clip_x),
                int(clip_y),
                int(max(0, clip_r - clip_x)),
                int(max(0, clip_b - clip_y)),
            )
        return None
            ...
            if target_area and target_region:
                ...
                if transitions_active and hit_op._container_data:
                    ...
                    for i, c in enumerate(hit_op._container_data):
                        cid = c.get("id", "")
                        for prefix, prop in [
                            ("_orig_bg_", "background_color"),
                            ("_orig_bc_", "border_color"),
                        ]:
